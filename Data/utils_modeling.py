...
def feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if {"AMT_ANNUITY", "AMT_CREDIT"}.issubset(df.columns):
        ...
        new_features["ANNUITY_TO_CREDIT"] = ratio3.replace([np.inf, -np.inf], np.nan)
    ...
    if all(col in df.columns for col in region_cols):
        ...
        new_features["REGION_MISMATCH_FLAG"] = (df[region_cols].sum(axis=1) > 0).astype(
            int
        )
    ...
    if "HOUR_APPR_PROCESS_START" in df.columns:
        conditions = [
            (df["HOUR_APPR_PROCESS_START"] >= 8) & (df["HOUR_APPR_PROCESS_START"] < 12),
    ...
    if "YEARS_BIRTH" in df.columns:
        ...
        new_features["AGE_GROUP"] = pd.cut(df["YEARS_BIRTH"], bins=bins, labels=labels)
    ...
    if {"DEF_30_CNT_SOCIAL_CIRCLE", "OBS_30_CNT_SOCIAL_CIRCLE"}.issubset(df.columns):
        ...
        new_features["SOCIAL_30_DPD_RATE"] = ratio8.replace([np.inf, -np.inf], np.nan)
    ...
    if {"DEF_60_CNT_SOCIAL_CIRCLE", "OBS_60_CNT_SOCIAL_CIRCLE"}.issubset(df.columns):
        ...
        new_features["SOCIAL_60_DPD_RATE"] = ratio9.replace([np.inf, -np.inf], np.nan)
    ...
    if housing_cols:
        new_features["HOUSING_QUALITY_SCORE"] = (
            df[housing_cols].astype("category").apply(lambda x: hash(tuple(x)), axis=1)
        )
    ...
    if "TARGET" in main_df.columns:
        ...
        if len(features) > 0 and y.nunique() > 1:
            ...
            analysis_df["combined_rank"] = (
                analysis_df["rank_importance"] * 0.6 + analysis_df["rank_roc_auc"] * 0.4
            )
...
def create_installment_custom_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["UNDERPAYMENT_FLAG"] = (df["UNDERPAYMENT_RATIO"] < 0.95).astype(int)
...
def create_bureau_custom_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    count_features["BUREAU_ACTIVE_CREDIT_COUNT"] = grp["HAS_ACTIVE_CREDIT"].sum()
...
def create_cc_balance_custom_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    temp_df["CREDIT_UTILIZATION_RATIO"] = np.where(
        ...
        temp_df["CREDIT_UTILIZATION_RATIO"].clip(0, 1.5),
        0,
    )
    ...
    temp_df["MADE_MINIMUM_PAYMENT"] = (temp_df["MIN_PAYMENT_RATIO"] >= 0.95).astype(int)
